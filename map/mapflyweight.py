...
class TileFlyWeight:
    def __init__(self, image:pygame.image) -> None:
       self.image =  image
       print(asizeof.asizeof(self.image))
...
#         self.rect.y = y
class Tile(pygame.sprite.Sprite):
    def __init__(self, flyweight:TileFlyWeight, x:int, y:int) -> None:
        """
        Creates a tile object
        ...
        for idx,row in enumerate(level):
            for idy,cell in enumerate(row):
                ...
                if cell in WALLS:#create the walls
                    ...
                    if image not in sharedflyweights:
                        sharedflyweights[image] = TileFlyWeight(image)
                    tile = Tile(sharedflyweights[image], idy*SIZE_X, idx*SIZE_Y)
                    sprites.add(tile)
                    ...
                    # tile = Tile(image, idy*SIZE_X + ((SIZE_X-x)/2), idx*SIZE_Y + (SIZE_Y-y))
                    if image not in sharedflyweights:
                        sharedflyweights[image] = TileFlyWeight(image)
                    tile = Tile(sharedflyweights[image], idy*SIZE_X + ((SIZE_X-x)/2), idx*SIZE_Y + (SIZE_Y-y))
